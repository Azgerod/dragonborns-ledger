...
* `docs/main-guide-writing-conventions.md`, if present, records the current player-facing prose standard for `main-guide-v1.md`.
...
7. `docs/main-guide-writing-conventions.md`, if present;
8. `docs/session-handoff.md`, if present.
...
The v1 guide must explicitly represent every row in `data/objectives/objectives.csv` and every checklist-relevant row already mapped in `data/checklist-mapping/coverage-matrix.csv`. A reader must not need to consult the objective spreadsheet, route-planning CSVs, support tables, source notes, or an external appendix to know what objective to do, where to do it, when to stop, what to collect, what to save/reload, or which completion state has changed.

Keep the player-facing guide distinct from internal coverage tracking. The guide should read like polished instructions for a Skyrim player. Do not use routine bracket checklist cues or repeated "mark this row" prompts for obvious quest completions, discoveries, spell/shout learns, book pickups, unique-item acquisitions, or similar actions; the guide's global policy tells the player to update the matching checklist row when those named actions occur. Use explicit player-facing bookkeeping only for non-obvious counters, randomized radiant results, branch-experienced rows, conditional availability, and route-control state. Do not expose objective IDs, coverage ledgers, row counts, "represented here" language, or coverage-matrix mechanics in normal guide prose unless the project later adopts an explicit final checklist-ID display format. Put objective IDs, row counts, staged/completed status, unresolved-row notes, and cue-to-objective mappings in `data/guide-coverage/main-guide-v1-coverage.csv` or another clearly internal coverage artifact.
...
At minimum, break the v1 work into one implementation pass per `MR-###` section, and split any large MR section further by quest chain, location corridor, support table, or checklist category when needed. Every pass must audit nearby safe objectives across all objective types, using both compiled project data and cited source pages, and route those objectives when they are safe and naturally colocated. For opening Survival segments, do not over-pull optional combat detours before the first settlement/support stop just because they are nearby; stabilize food, rest, carry weight, and save state first unless there is a strong route reason. Do not assume early house, AE-home, mount, pet, armor, training, or furnishing money that the route has not actually generated through normal rewards, looting, and selling; unlocking a purchase surface is not the same as buying and verifying it. For multi-location quests, time the quest start relative to the next routed progress point, usually on the latest efficient visit to the start location before progress or completion. Keep related objectives together as coherent bundles where possible; avoid shallow "discover now, do later" steps unless the partial action has a concrete route reason. Do not ask the player to manipulate RNG or reroll ordinary radiant targets; respect the active marker and write conditional route branches. Follow `docs/main-guide-v1-expansion-plan.md` as the active work breakdown. For player-facing prose style, follow `docs/main-guide-writing-conventions.md`. Do not move on to TB-036 appendices/reference tables until the v1 self-contained guide expansion is complete or the user explicitly re-scopes the sequence.
