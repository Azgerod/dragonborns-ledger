...
Phase 1 source-list objective setup is closed. The objective database currently has 2,789 rows, and source notes currently run through `SN-000129-checklist-source-readiness-forward-review.md`.
...
TB-035-MR-002 is complete. MR-002 now uses the corrected two-layer model: player-facing prose in `drafts/final-guide/main-guide-v1.md`, internal objective/status mapping in `data/guide-coverage/main-guide-v1-coverage.csv`. The guide section routes the Helgen escape, post-Helgen Survival prompt, Helgen/Guardian Stones/Riverwood discovery cues, Warrior Stone/Blessed cue, Hadvar/Alvor opening default, Alvor's woodcutter's axe, Riverwood food/rest support, staged `Before the Storm`, `The Golden Claw` start at Riverwood Trader, `A Lovely Letter` in Faendal's favor with both forged letters, Faendal follower/steward staging, Embry's drink-favor representative, and Hero of the People 1/50. It holds Treasure Map I and its Riverwood chest for the early economy/Riverwood loop because optional bandit combat before the first support stop is not worth the opening-stabilization cost. It also holds Embershard Mine silently for a later Smith 'N Slash / Orcish Plate / clearable-location bundle instead of doing a shallow discovery-only pass.

TB-035-MR-003 is complete. The player-facing guide no longer exposes MR codes in route headings. The first Whiterun section is now `First Day In Whiterun` and routes a geographic city loop: Whiterun Stables/Whiterun/Dragonsreach discovery, `Before the Storm`, Farengar Dragonstone objective, Adrianne/Proventus greatsword, Farengar/Arcadia frost salts, Carlotta/Mikael, Ysolda and Amren starts, Brenuin beggar plus Argonian Ale, Lars/Braith, Andurs/Hall of the Dead Stone, Hulda chop wood, Uthgerd brawl, Hero of the People 7/50, Hard Worker wood 1/3, and conditional Snake Tongue Persuade tracking. Broad unrouted Whiterun content is handled by Route Discipline and internal coverage rather than repeated player-facing prohibition lists.
...
After user approval, start TB-035-MR-008 by expanding only the `Goldenhills Plantation` section in `drafts/final-guide/main-guide-v1.md`. Use corrected setup, Helgen/Riverwood, First Day In Whiterun, Bleak Falls/first-dragon, Saadia/first-horse, Guardian Stones/farm, and Halted Stream/Silent Moons sections as the model: player-facing prose in the guide, natural checklist cues, and objective/checklist IDs plus coverage status in `data/guide-coverage/main-guide-v1-coverage.csv`.

Primary output:

* updated `drafts/final-guide/main-guide-v1.md` with the Goldenhills Plantation section expanded as polished player-facing central/Rorikstead instructions
* natural checklist cues for `The Unquiet Dead`, Goldenhills ownership, property systems, farm staffing, food/income/support functions, and any nearby objectives that belong in the same pass
* updated `data/guide-coverage/main-guide-v1-coverage.csv` with TB-035-MR-008 objective/checklist IDs, coverage status, staged/completed status, and unresolved rows
* task-board and handoff status updates when the TB-035-MR-008 pass is complete
