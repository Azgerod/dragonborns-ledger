...
TB-035-MR-005 is complete and pending user review. The section is now `Saadia, First Horse, And Western Road Support`. It completes `In My Time Of Need` on the Saadia-default path, buys the Whiterun stable horse, applies Elven Horse Armor, discovers and quest-sweeps Swindler's Den, leaves the local `Thief` skill-book copy closed, cooks one food item for Hard Worker 2/3, advances Sideways to 2/10, and explicitly stages Breezehome/Tundra purchase until normal route economy can fund them. `sources/source-notes/SN-000133-post-dragon-whiterun-western-road.md` supports the new route details. Internal coverage tracks 19 MR-005 objective/option rows with one Swindler's Den clear-state route-resolution note.

The current next step is user review of the expanded `Saadia, First Horse, And Western Road Support` section. TB-035-MR-006 is blocked until the user approves or revises this route style. If approved, expand `Early Local Economy And Riverwood Loop`: build the Breezehome fund without hidden grind/exploit assumptions, route Treasure Map I and its Riverwood chest after support stabilization, acquire a pickaxe and mine ore for Hard Worker completion, handle the crop-sale representative favor, decide whether Embershard/local Riverwood-Whiterun objectives belong there as coherent bundles, and keep Tundra Homestead as a later 7,500-gold main-base target unless the route has actually generated the funds. `docs/main-guide-writing-conventions.md` is the live style reference for future guide-writing passes.

Route section boundaries are adjustable during TB-035. If objective-level review shows that the old MR scaffold splits content poorly, move or split the route jurisdiction and update `main-guide-v1.md`, `data/guide-coverage/main-guide-v1-coverage.csv`, `docs/main-guide-v1-expansion-plan.md`, `docs/task-board.md`, and this handoff instead of forcing objectives into stale boundaries. Every section pass must run the nearby-objective audit in `docs/main-guide-writing-conventions.md` across all objective types, use both compiled data and source pages, route safe colocated objectives, keep related objectives bundled where possible, and time multi-location quest starts relative to the next routed progress point. In opening Survival segments, optional combat detours can wait until after food/rest/carry/save stabilization even when they are nearby. Do not assume early property or AE-home money that the route has not generated through normal rewards, looting, and selling; unlocking a purchase surface is not the same as buying and verifying storage.
...
After user approval, start TB-035-MR-006 by expanding only the `Early Local Economy And Riverwood Loop` section in `drafts/final-guide/main-guide-v1.md`. Use corrected setup, Helgen/Riverwood, First Day In Whiterun, Bleak Falls/first-dragon, and Saadia/first-horse sections as the model: player-facing prose in the guide, natural checklist cues, and objective/checklist IDs plus coverage status in `data/guide-coverage/main-guide-v1-coverage.csv`.

Primary output:

* updated `drafts/final-guide/main-guide-v1.md` with the Early Local Economy And Riverwood Loop section expanded as polished player-facing Whiterun/Riverwood economy and nearby-objective instructions
* natural checklist cues for Treasure Map I, Treasure Chest I, Embershard/local objectives that belong in the bundle, remaining Hard Worker mining/trophy completion, representative crop sale, early Artificer/crafting setup if it belongs here, and Breezehome purchase/storage verification only if the route has actually generated the gold
* updated `data/guide-coverage/main-guide-v1-coverage.csv` with TB-035-MR-006 objective/checklist IDs, coverage status, staged/completed status, and unresolved rows
* task-board and handoff status updates when the TB-035-MR-006 pass is complete
