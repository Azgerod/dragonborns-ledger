...
| Objective rows | All 2,789 rows still have `research_status=needs_review`; validation is `not_started` for 2,769 and `needs_review` for 20. | Leave unchanged. These are database-wide inventory/QA labels, not final-route blockers. TB-033 validated the prototype layer, TB-034 placed the minimal route, and TB-035 consumes objective rows only after inspecting citations and constraints. |
...
| Property support rows | 52 rows retain `safe_storage_status=needs_validation_later`; 7 Goldenhills notes named TB-031H/TB-035. | Goldenhills notes now say TB-031H audited ownership and TB-035 owns option/final-guide validation. Other property rows now have TB-032 warning triggers where supported; TB-034 placed minimal route containers, while TB-035/TB-037 still own option or final storage validation by use. |
...
| Objective-wide validation status | 2,789 objectives | TB-033 complete at prototype level; TB-034/TB-037 by final step use | Final step validation depends on route ordering, branches, counters, rewards, and Survival support. |
...
| Property/storage validation | 52 `needs_validation_later` property-detail rows | TB-035/TB-037 by use after TB-032 warning overlay and TB-034 minimal placement | Warning triggers and route containers are now recorded where supported; option presentation and final storage validation remain separate downstream tasks. |
...
No unresolved TB-031H blocker remains, TB-031J removed the checklist source-readiness queue before TB-032, TB-031K refreshed downstream planning artifacts, TB-032 recorded the warning/hard-save trigger layer, TB-033 validated the prototype, and TB-034 placed the minimal numbered route. The remaining risk is ordinary final-guide drafting and final-QA risk: TB-035 may need to adjust prose or option presentation if expansion exposes a concrete contradiction, and TB-037 through TB-043 perform final observed-counter, final skill-state, storage, trophy-pop, and playtest validation.
