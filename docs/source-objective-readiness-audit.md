...
Status: TB-031H complete; TB-031J source-readiness rows pulled forward; TB-031K downstream refresh complete. TB-035 is reopened for self-contained v1 guide expansion; see `docs/main-guide-v1-expansion-plan.md`.
...
| Objective rows | All 2,789 rows still have `research_status=needs_review`; validation is `not_started` for 2,769 and `needs_review` for 20. | Leave unchanged. These are database-wide inventory/QA labels, not final-route blockers. TB-033 validated the prototype layer and TB-034 placed the minimal route. Reopened TB-035 v1 must consume objective rows into explicit guide instructions without treating broad readiness labels as blockers. |
| Generated prototype map | Pre-audit rows still named TB-031H as an owner for dependency/support placement. | `tools/build_prototype_objective_block_map.py` now assigns unresolved dependency/support rows to named downstream owners such as TB-034, TB-035, TB-036, or TB-037. Regenerated CSV has 0 `TB-031H` `deferred_to` values. |
...
| Progression source-selection rows | Investment rows for bugged or unknown merchants still mentioned TB-031H/TB-033. | Generator assigns those rows to investment validation; TB-033 kept bugged/unknown rows audit-only rather than routing them as required investments. |
| Property support rows | 52 rows retain `safe_storage_status=needs_validation_later`; 7 Goldenhills notes named TB-031H/TB-035. | Goldenhills notes now say TB-031H audited ownership and TB-035 owns option/final-guide validation. Other property rows now have TB-032 warning triggers where supported; TB-034 placed minimal route containers. Reopened TB-035 v1 must add explicit property/service/storage/display instructions where routeable, while TB-037 still owns final observed storage validation by use. |
| Book and spell support rows | Many rows remain `source_listed_candidate` because they are candidate sources, not chosen route steps. | Leave as candidate data. TB-031E chose progression source policy; TB-031J resolved checklist-only book source-readiness rows; reopened TB-035 v1 must consume selected book/spell/document sources into the guide, and TB-037 performs final checklist verification. |
| Location support rows | `location-catalog.csv` has no remaining `needs_research` access rows; `route_status=source_listed_candidate` remains normal for 437 locations. | Leave as candidate geography data. TB-031G resolved access/clear/discovery mechanics; TB-032 placed warnings and TB-034 places final route steps. |
| Merchant investment support rows | 33 available, 13 bugged/unofficial-patch-only, 4 `unknown_needs_validation`. | Available rows remain in the investment circuit; TB-033 kept bugged/unknown rows non-required/audit-only unless a later final-route test proves official PS4 AE availability. |
...
| Objective-wide validation status | 2,789 objectives | TB-033 complete at prototype level; TB-035 v1/TB-037 by final step use | Final step validation depends on route ordering, branches, counters, rewards, and Survival support. |
...
| Property/storage validation | 52 `needs_validation_later` property-detail rows | TB-035 v1 for explicit instructions; TB-037 for observed validation by use | Warning triggers and route containers are recorded where supported; final storage validation remains separate downstream QA. |
| Unknown/bugged merchant investments | 17 non-routed investment rows | TB-037 only if a final route attempts to promote them | Official PS4 AE availability or bug behavior must be validated before any route requirement. |
...
No unresolved TB-031H blocker remains, TB-031J removed the checklist source-readiness queue before TB-032, TB-031K refreshed downstream planning artifacts, TB-032 recorded the warning/hard-save trigger layer, TB-033 validated the prototype, and TB-034 placed the minimal numbered route. `main-guide-v0.md` is scaffold prose only; reopened TB-035 v1 must now convert objective/support/checklist rows into explicit self-contained guide instructions. TB-036 appendices are blocked until that v1 coverage exists or the user re-scopes the sequence, and TB-037 through TB-043 remain final observed-counter, final skill-state, storage, trophy-pop, and playtest validation.
