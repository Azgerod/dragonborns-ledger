...
Status: TB-031G complete as a route-planning validation layer; TB-034 has consumed it into the minimal route prototype.

Scope: this file does not write final route prose or pathfinding. It records how location rows must be interpreted after TB-032 warnings and TB-033 validation. TB-034 used these rules for prototype placement; TB-035/TB-037 still own final guide prose and final counter validation.
...
| --- | --- | --- |
| Delver / clearable rows | Source-listed clearable rows count for `Delver` when they increment `Dungeons Cleared`; Angarvunde and Mistwatch remain atypical non-counting clears. | TB-033 validated prototype handling; TB-034 placed cleanup containers; TB-037 validates final counter totals. |
| Clear trigger wording | Normal clearables should be routed until the in-game `Cleared` tag appears, usually after boss defeat; active `Kill the Bandit Leader` targets may require report-back before the tag. | TB-032 warning placement; TB-034 prototype placement; TB-035 final guide steps. |
| Explorer / discovery rows | Source-listed discoverable rows are independent discovery/checklist rows. Duplicate markers can count for Explorer but not as extra Delver clears. | TB-033 validated prototype handling; TB-034 placed route containers; TB-037 validates `Locations Discovered`. |
