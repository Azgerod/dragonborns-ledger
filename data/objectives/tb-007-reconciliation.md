...
Status: TB-007F complete; TB-007G3 complete.
...
| Books and documents policy | Current required title rows have acquisition/location candidate rows. Duplicate copies are route candidates, not separate objectives. | Re-open only if checklist mapping supplies unique book/note rows not already represented. |
...
| Final aggregate reconciliation | `objectives.csv`, `data/books/`, `data/npc/`, and new member tables | Confirm each finite aggregate row is expanded, linked to a candidate table, delegated to a downstream constraint task, or explicitly justified as parent-only. |

## Result

No obvious additional TB-007 source-list category gap is open after reconciling powers, books/documents, unique items/rewards, finite collectibles, properties, pets, mounts, followers, and role assignments.

The next project step should be TB-007G4: reconcile remaining aggregate parent/set rows before objective-database completeness review.
