...
| Source-readiness holds | Unchanged at 90; quest source-readiness rows now point to TB-031F/TB-031H after this escalation review. |
...
| Ghosts of the Tribunal | Keep join/infiltrate as main continuity and destroy-heretics as the branch audit. Checklist book/document rows that are not source-ready remain TB-031H/TB-036 work; no extra branch is promoted here. | TB-031H validates source-readiness rows; TB-033 verifies final branch/checklist coverage. |
...
| TB-031B quest source-readiness rows | Do not promote `Rebuilding the Blades`, `Dragon Hunting`, `Archery Practice`, or `Scare My Enemy` without source validation. Blades rows can attach to BR-004 only if TB-031F/TB-031H validates them as source-backed and compatible with the Paarthurnax branch. | TB-031F/TB-031H. |
...
| Exact source-readiness validation | TB-031H audits source/objective/support readiness after TB-031D through TB-031G touch their rows. |
