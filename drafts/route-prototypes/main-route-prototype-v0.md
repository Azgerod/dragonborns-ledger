...
| TB-031F source-readiness rows | `Rebuilding the Blades` and `Dragon Hunting` are branch-only Blades rows; `Archery Practice`, `Scare My Enemy`, `Firebrand Wine Case`, and `Map of Dragon Burials` now have source-backed main-route handling. | TB-031H audited metadata; TB-031J resolves the remaining source-readiness rows; TB-033 validates branch/main continuity. |
